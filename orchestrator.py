...
def _deployment_base_url() -> str:
    """Public base URL when agents are mounted on one host (e.g. Vercel)."""
    base = (os.environ.get("AGENT_BASE_URL") or os.environ.get("VERCEL_URL") or "").strip().rstrip("/")
    if not base:
        return ""
...
def get_agent_registry() -> dict:
    ...
    if base:
        return {
            "research-agent-001": {
                "base_url": f"{base}/agents/research",
                "token":    os.environ.get("AGENT_TOKEN_RESEARCH", "research-token-abc123"),
                "card_url": f"{base}/agents/research/.well-known/agent.json",
            },
            "writer-agent-002": {
                "base_url": f"{base}/agents/writer",
                "token":    os.environ.get("AGENT_TOKEN_WRITER", "writer-token-def456"),
                "card_url": f"{base}/agents/writer/.well-known/agent.json",
            },
            "analyst-agent-003": {
                "base_url": f"{base}/agents/analyst",
                "token":    os.environ.get("AGENT_TOKEN_ANALYST", "analyst-token-ghi789"),
                "card_url": f"{base}/agents/analyst/.well-known/agent.json",
            },
        }
    return {
        "research-agent-001": {
...
AUTH_SERVICE_URL = os.environ.get("AUTH_SERVICE_URL", "http://127.0.0.1:8004").rstrip("/")
...
async def proxy_login(req: dict):
    async with httpx.AsyncClient() as client:
        resp = await client.post(f"{AUTH_SERVICE_URL}/auth/login", json=req)
        return resp.json() if resp.status_code == 200 else {"error": resp.text}

@app.post("/auth/signup")
async def proxy_signup(req: dict):
    async with httpx.AsyncClient() as client:
        resp = await client.post(f"{AUTH_SERVICE_URL}/auth/signup", json=req)
        return resp.json() if resp.status_code == 200 else {"error": resp.text}
